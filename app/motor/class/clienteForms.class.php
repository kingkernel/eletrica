...
class clienteForms {
	...
	//	$hidden2 = '<input type="hidden" name="iduser" value="'.$_SESSION["userinfo"]["id"].'">';
		$submit = '<input type="submit" value="cadastrar" class="btn btn-primary pull-right" style="margin:10px" id="cadAssessorBtn">';
			...
			$this->cadIn[1] = '';
			$this->cadIn[2] = '$("#id_'.$fone1->inputName.'").mask(\'(99) 99999-9999\');$("#id_'.$nasc->inputName.'").mask(\'(99) 99999-9999\');$("#id_'.$zap->inputName.'").mask(\'(99) 99999-9999\');$("#cadAssessorBtn").click(function(){/* variaveis */var nome = $("#id_'.$nome->inputName.'").val();var cidade = $("#id_'.$city->inputName.'").val();var bairro = $("#id_'.$bairro->inputName.'").val();var rua = $("#id_'.$rua->inputName.'").val();var numero = $("#id_'.$numero->inputName.'").val();var email = $("#id_'.$email->inputName.'").val();var fone1 = $("#id_'.$fone1->inputName.'").val();var fone2 = $("#id_'.$nasc->inputName.'").val();var zap = $("#id_'.$zap->inputName.'").val();var hidden1 = "'./*$_SESSION["userinfo"]["idlicenca"].*/'"; var iduser = "'./*$_SESSION["userinfo"]["id"].*/'";/* autenticação de campos *//* nome do eleitor */if(nome == ""||nome==null){alert("é preciso o nome do eleitor");$("#id_'.$nome->inputName.'").addClass("white label-warning");$("#id_'.$nome->inputName.'").focus();return false;};/* cidade */if(cidade == ""|| cidade ==null){alert("Qual a cidade do Eleitor?");$("#id_'.$city->inputName.'").addClass("white label-warning");$("#id_'.$city->inputName.'").focus();return false;}; /* envio de dados */$("#ajax").load("'.(urlcss($_GET)).'grava/assessor/",{eleitor:nome, cidade: cidade,bairro: bairro, rua: rua, numero:numero, email:email, fone1:fone1, fone2:fone2, zap:zap, licenca:hidden1, iduser:iduser});});';
			return $this->cadIn;
	}
